...
exports.commands = {
	...
	popup: function (target, room, user) {
		...
		let popupSender = this.targetUserOrSelf();
		this.logModCommand(popupSender + " sent " + targetUser + " a popup message.");
		targetUser.send('|popup|' + target);
	}, 
	popuphelp: ["/popup OR /pop OR /pup [username], [message] - Displays a popup with a message to a user. Requires: & ~"],
	'!authority': true,
	auth: 'authority',
	stafflist: 'authority',
